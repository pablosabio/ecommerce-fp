...
const fixDatabase = async () => {
  try {
    ...
    const problematicIndex = indexes.find(
      (index) => index.key && index.key.paymentIntentId === 1 && index.unique
    );
    ...
    if (problematicIndex) {
      ...
      await db
        .collection('orders')
        .createIndex({ paymentIntentId: 1 }, { unique: true, sparse: true });
    ...
    const nullOrders = await db.collection('orders').find({ paymentIntentId: null }).toArray();
    ...
    if (nullOrders.length > 1) {
      ...
      for (let i = 1; i < nullOrders.length; i++) {
        await db
          .collection('orders')
          .updateOne({ _id: nullOrders[i]._id }, { $unset: { paymentIntentId: '' } });
      }

      console.log(
        `Fixed ${nullOrders.length - 1} orders by removing the null paymentIntentId field.`
      );
    }
